# -*- coding: utf-8 -*-
# DHTM :Dual Hierarchical Topic Model (based on LLDA )

# LLDA:L-LDA(Labeled Latent Dirichlet Allocation Model)
# @Author: Jiahong Zhou
# @Date: 2018-10-20
...
class DhtmModel:
    ...
    @field W: the corpus, a list of terms list,
              W[m] is the document vector, W[m][n] is the id of the term #词袋
    @field Z: the topic corpus, just same as W,#主题词袋
    ...
    def __init__(self, alpha_vector="50_div_K", eta_vector=None, labeled_documents=None):
        ...
        self.pi_vector=None  
        self.features=enum 
        self.FN=len(self.features)         
        self.feature_vocabulary={} 
        self.L=[] 
        self.Lambda_LM=None #mask

        self.pi_vector_Lambda=None
        self.class_vector =None

        self.Topic2FeatureCount=None
        self.Doc2FeatureCount=None
        ...
        self.Doc2FeatureCountSum=None
        self.labels_exist_np=None
    ...
    def _load_labeled_documents(self, labeled_documents):
        ...
        ChemoTherapy_all=[]
        RadioTherapy_all=[]
        EndocrineTherapy_all=[]
        TargetedTherapy_all=[]
                       
        for document, labels, label_exist in labeled_documents:
            document = DhtmModel._document_preprocess(document)  
            ...
            ChemoTherapy_all.append(labels[0])
            RadioTherapy_all.append(labels[1])
            EndocrineTherapy_all.append(labels[2])
            TargetedTherapy_all.append(labels[3])
            labels_corpus.append(labels)
            
            all_words.extend(doc_words)
            all_labels.extend(labels)
            labels_exist.append(label_exist[:4])
            
        self.ChemoTherapy=list(set(ChemoTherapy_all))
        self.RadioTherapy=list(set(RadioTherapy_all))
        self.EndocrineTherapy=list(set(EndocrineTherapy_all))
        self.TargetedTherapy=list(set(TargetedTherapy_all))  
        
        self.labels_exist=labels_exist
        self.terms = list(set(all_words)) #token set
        self.vocabulary = {term: index for index, term in enumerate(self.terms)}
        self.index_2_term={index: term for index, term in enumerate(self.terms)}
        ...
        self.M = len(self.W)
        self.WN = len(all_words)#including repetitive words
        # LN is the number of original labels, including repetitive labels
        self.LN = len(all_labels) - self.M
                    
        """
            pi_vector  :FN  
            alpha_vector :K  eta_vector:T class_vector:(4)
        """
        ...
        if self.delta_vector is None:
            self.delta_vector = [0.001 for _ in range(self.FN)]
        elif type(self.delta_vector) is float or type(self.delta_vector) is int:
            self.delta_vector = [self.delta_vector for _ in range(self.FN)]
        else:
            message = "error delta_vector: %s" % self.delta_vector
            raise Exception(message)
        
        if self.class_vector is None:
            self.class_vector = [0.001 for _ in range(4)]
        elif type(self.class_vector) is float or type(self.class_vector) is int:
            self.class_vector = [self.class_vector for _ in range(4)]
        else:
            message = "error class_vector: %s" % self.class_vector
            raise Exception(message)
        
        if self.pi_vector is None:
            self.pi_vector = [0.001 for _ in range(self.FN)]
        elif type(self.pi_vector) is float or type(self.pi_vector) is int:
            self.pi_vector = [self.pi_vector for _ in range(self.FN)]
        else:
            message = "error pi_vector: %s" % self.pi_vector
            raise Exception(message)
        ...
        #mask K*T 
        self.Lambda_topic2term = np.zeros((self.K, self.T), dtype=float)
        for m in range(self.M):
            for label in labels_corpus[m]: 
                k = self.topic_vocabulary[label] 
                for t in self.W[m]:
                    self.Lambda_topic2term[k,t]=1
                    
        #mask self.Lambda_LM M*FN 
        self.feature_vocabulary={feature: index for index,feature in enumerate(self.features)}
        self.Lambda_LM = np.zeros((self.M, self.FN), dtype=float)
        for m in range(self.M):
            for word in doc_corpus[m]:
                for feature in self.features:
                    if feature == word.split(':')[0]:
                        l=self.feature_vocabulary[feature]
                        self.Lambda_LM[m,l]=1

        #mask self.Lambda_doc2classtopic        
        self.Lambda_doc2classtopic = np.zeros((self.M, self.K), dtype=float)
        self.labels_exist_np=np.array(labels_exist)
        for m in range(self.M):
            if self.labels_exist_np[m][0]==1:
                for j in range(len(self.ChemoTherapy)):
                    self.Lambda_doc2classtopic[m][self.topic_vocabulary[self.ChemoTherapy[j]]]=1
            if self.labels_exist_np[m][1]==1:
                for j in range(len(self.RadioTherapy)):
                    self.Lambda_doc2classtopic[m][self.topic_vocabulary[self.RadioTherapy[j]]]=1
            if self.labels_exist_np[m][2]==1:
                for j in range(len(self.EndocrineTherapy)):
                    self.Lambda_doc2classtopic[m][self.topic_vocabulary[self.EndocrineTherapy[j]]]=1
            if self.labels_exist_np[m][3]==1:
                for j in range(len(self.TargetedTherapy)):
                    self.Lambda_doc2classtopic[m][self.topic_vocabulary[self.TargetedTherapy[j]]]=1
        
        #mask 4*K  
        self.Lambda_class2topic=np.zeros((4,self.K), dtype=float)
        for j in range(len(self.ChemoTherapy)):
            self.Lambda_class2topic[0][self.topic_vocabulary[self.ChemoTherapy[j]]]=1
        for j in range(len(self.RadioTherapy)):
            self.Lambda_class2topic[1][self.topic_vocabulary[self.RadioTherapy[j]]]=1
        for j in range(len(self.EndocrineTherapy)):
            self.Lambda_class2topic[2][self.topic_vocabulary[self.EndocrineTherapy[j]]]=1            
        for j in range(len(self.TargetedTherapy)):
            self.Lambda_class2topic[3][self.topic_vocabulary[self.TargetedTherapy[j]]]=1   


        for m in range(self.M):
            temp_list=[]
            for n in range(len(self.W[m])):
                word=self.index_2_term[self.W[m][n]]
                for feature in self.features:
                    if feature == word.split(':')[0]:
                        l=self.feature_vocabulary[feature]
                        temp_list.append(l)
                        break
            assert len(temp_list) == len(self.W[m])                
            self.L.append(temp_list)  
          

        self.TopicClass=[] 
        for m in range(self.M):
            p=[0.001 for _ in range(4)]+self.labels_exist_np[m]
            p_dir=np.random.dirichlet(alpha=p,size=1)
            while(pd.isna(p_dir[0]).any() or np.isinf(p_dir[0]).any()):
                p_dir=np.random.dirichlet(alpha=p,size=1)
            z_vector=[DhtmModel._multinomial_sample(p_dir[0]) for _ in range(len(self.W[m]))]
            self.TopicClass.append(z_vector)                            
        ...
        for m in range(self.M):
            ...
            z_vector = [DhtmModel._multinomial_sample(p_vector) for _ in range(len(self.W[m]))]
    ...
    def _initialize_derivative_fields(self):
        ...
        self.Doc2FeatureCount = np.zeros((self.M, self.FN), dtype=int)
        self.Topic2FeatureCount=np.zeros((self.K,self.FN),dtype=int)       
        self.Doc2TopicClass =np.zeros((self.M, 4), dtype=int)

        for m in range(self.M):
            for t,c,l, z in zip(self.W[m],self.TopicClass[m],self.L[m],self.Z[m]):    
                k = z
                self.Doc2FeatureCount[m,l]+=1
                self.Doc2TopicCount[m, k] += 1
                self.Doc2TopicClass[m,c]+=1
                self.Topic2TermCount[k, t] += 1
                self.Topic2FeatureCount[z,l]+=1
        ...
        self.pi_vector_Lambda=self.Lambda_LM*self.pi_vector
        self.Topic2TermCountSum = self.Topic2TermCount.sum(axis=1)
        self.Doc2FeatureCountSum=self.Doc2FeatureCount.sum(axis=1)#S
        
    @staticmethod
    def _multinomial_sample(p_vector, random_state=None):
        """
        sample a number from multinomial distribution
        :param p_vector: the probabilities #P是主题概率（57,)
        :return: a int value
        """
        if random_state is not None:
            t=random_state.multinomial(1, p_vector)
            return random_state.multinomial(1, p_vector).argmax()
        #t=np.random.multinomial(1, p_vector)
    ...
    def _gibbs_sample_training(self):
        ...
        count=0
        very_small=0.00000000001
        for m in range(self.M): #M   #文章数
            doc_m_alpha_vector = self.alpha_vector_Lambda[m]
            #for t,topicClass,l,z, n in zip(self.W[m],self.TopicClass[m], self.L[m],self.Z[m], range(len(self.W[m]))):#W[m] is the document vector 一个词袋, W[m][n] is the id of the term
            for t,topicClass,l,z, n in zip(self.W[m],self.TopicClass[m],self.L[m], self.Z[m], range(len(self.W[m]))):#W[m] is the document vector 一个词袋, W[m][n] is the id of the term
                k = z                                  #Z[m][n] is the id of the topic of the term
                self.Doc2TopicCount[m, k] -= 1 
                self.Topic2TermCount[k, t] -= 1
                self.Topic2TermCountSum[k] -= 1
                self.Doc2FeatureCount[m,l] -= 1
                self.Doc2TopicClass[m,topicClass]-=1
                self.Topic2FeatureCount[k,l]-=1
                self.Doc2FeatureCountSum[m]-=1
                ...
                numerator_pi_vector = self.Doc2FeatureCount[m,l] + self.delta_vector[l]
                denominator_pi = self.Doc2FeatureCount[m].sum() + sum(self.delta_vector)
                pi_vector=1.0 *numerator_pi_vector / denominator_pi
                
                P_wl=beta_vector*pi_vector
                
                numerator_theta_vector = (self.Doc2TopicCount[m] + self.alpha_vector)*self.Lambda[m]*self.Lambda_doc2classtopic[m]+very_small
                denominator_theta=self.Doc2TopicCount[m].sum()-1+(self.alpha_vector*self.Lambda[m]*self.Lambda_doc2classtopic[m]).sum()+very_small
                theta_vector = numerator_theta_vector/denominator_theta

                p_vector = P_wl * theta_vector
                p_vector = 1.0 * p_vector / sum(p_vector)
                
                sample_z = DhtmModel._multinomial_sample(p_vector)
                self.Z[m][n] = sample_z
                k = sample_z
                
                if k in self.ChemoTherapy:
                    topicClass=0
                elif k in self.RadioTherapy:
                    topicClass=1
                elif k in self.EndocrineTherapy:
                    topicClass=2
                elif k in self.TargetedTherapy:
                    topicClass=3
                self.Doc2TopicCount[m, k] += 1
                self.Topic2TermCount[k, t] += 1
                self.Topic2TermCountSum[k] += 1
        ...
        print('count:self.WN,',count,self.WN)
    ...
    def _gibbs_sample_inference(self, term_vector, iteration=300, times=10):
        ...
        z_vector = [DhtmModel._multinomial_sample(p_vector) for _ in term_vector]#对每一个单词进行一个主题的采样#初始化
        z_class_vector=[]
        for k in z_vector:
            if k in self.ChemoTherapy:
                topicClass=0
                ...
                topicClass=3
            z_class_vector.append(topicClass)
        
        doc_topic_class_count = np.zeros(self.KN, dtype=int)
        accumulated_doc_topic_class_count = np.zeros(self.KN, dtype=int)

        doc_feature_count=np.zeros(self.FN)
        feature_list=[]
        for n in range(len(term_vector)):
            word=self.index_2_term[term_vector[n]]
            #print(n,word)
            for feature in self.features:
                if feature == word.split(':')[0]:
                    l=self.feature_vocabulary[feature]
                    ...
                    break        
        #print('len(feature_list),len(term_vector):',len(feature_list),len(term_vector))
        assert len(feature_list) == len(term_vector)                

        doc_feature_count_sum=0
        for n,t in enumerate(term_vector):
            k = z_vector[n] 
            topicClass=z_class_vector[n]
            l=feature_list[n]
            ...
            self.Topic2FeatureCount[k,l]+=1
            doc_topic_count[k] += 1
            doc_feature_count[l]+=1
            doc_topic_class_count[topicClass]+=1
            doc_feature_count_sum+=1
        doc_m_alpha_vector = self.alpha_vector #对的
        ...
        for i in range(iteration+times):
            for n, t in enumerate(term_vector): 
                ...
                numerator_pi_vector = doc_feature_count[l] + self.delta_vector[l]
                denominator_pi = doc_feature_count_sum + sum(self.delta_vector)
                pi_vector=1.0 *numerator_pi_vector / denominator_pi
                
                P_wl=beta_vector*pi_vector

                numerator_theta_vector = (doc_topic_count + self.alpha_vector)+very_small
                denominator_theta=sum(doc_topic_count)-1+sum(self.alpha_vector)+very_small
                theta_vector = numerator_theta_vector/denominator_theta
                p_vector = P_wl * theta_vector
                ...
                for key,value in self.topic_vocabulary.items():
                    if value==k:
                        topic_k=key
                if topic_k in self.ChemoTherapy:
                    topicClass=0
                elif topic_k in self.EndocrineTherapy:
                    topicClass=1
                elif topic_k in self.RadioTherapy:
                    topicClass=2
                elif topic_k in self.TargetedTherapy:
                    topicClass=3
    ...
    #     p_vector = np.ones(self.K, dtype=int)
    #     p_vector = p_vector * 1.0 / sum(p_vector)
    #     z_vector = [DhtmModel._multinomial_sample(p_vector, random_state=random_state) for _ in term_vector]
    #     for n, t in enumerate(term_vector):
    #         k = z_vector[n]
    #         doc_topic_count[k] += 1
    ...
    #             p_vector = 1.0 * p_vector / sum(p_vector)
    #             # print p_vector
    #             sample_z = DhtmModel._multinomial_sample(p_vector, random_state)
    #             z_vector[n] = sample_z
    #
    #             k = sample_z
    #             doc_topic_count[k] += 1
    ...
    def training(self, iteration=10, log=False):
        ...
        for i in range(iteration):
            ...
            self._gibbs_sample_training()#进行采样 
        pass

    def inference(self, document, iteration=30, times=10):#对一篇文章进行概率的推断。。。
        ...
        sorted_doc_topic_new=  theta_new 
...
##是单词t从第k个topic生成的概率
    ...
    def log_perplexity(self, documents=None, iteration=30, times=10):
        ...
        if not documents:
            ...
            documents = [DhtmModel._document_preprocess(document) for document in documents]
    ...
    def _write_object_to_file(file_name, target_object):
        ...
        DhtmModel._find_and_create_dirs(dirname)
        try:
    ...
    def save_model_to_dir(self, dir_name, save_derivative_properties=False):
        ...
        :return: None
        """
        save_model = DhtmModel.SaveModel()
        ...
        DhtmModel._write_object_to_file(save_model_path, save_model.__dict__)
    ...
    def load_model_from_dir(self, dir_name, load_derivative_properties=True):
        ...
        """
        save_model_path = os.path.join(dir_name, "llda_model.json")
        save_model_dict = DhtmModel._read_object_from_file(save_model_path)
        save_model = DhtmModel.SaveModel(save_model_dict=save_model_dict)
    ...
    def top_terms_of_topic(self, topic, k, with_probabilities=True):
        ...
        beta = self.beta_k(self.topic_vocabulary[topic])  #返回一个形状为T的矩阵，是单词的数量，beta_k[t]是这个单词属于这个主题的概率
